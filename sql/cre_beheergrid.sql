/* query time: 1m 51s
 * 
 *  Intersectie met Bgt-BRK (Beheerkaart - Publieke Ruimte) met cbs-grid (100x100)
 * 
 ...
 */
drop table if exists "openbare_ruimte_CROW".opr_grid;
CREATE TABLE "openbare_ruimte_CROW".opr_grid as 
(
select
	grid.id,
	grid.col,
...
--
	...
	from    (
			...
			join 
				"openbare_ruimte_CROW".brk_bgt as layer2
			on 
...
--
drop index if exists "openbare_ruimte_CROW".opr_grid_gindx;
CREATE INDEX opr_grid_gindx ON "openbare_ruimte_CROW".opr_grid USING GIST (geometrie);
create index opr_grid_fractie_idx on "openbare_ruimte_CROW".opr_grid(fractie);
vacuum analyze "openbare_ruimte_CROW".opr_grid;
--
select Populate_Geometry_Columns('"openbare_ruimte_CROW".opr_grid'::regclass, true);