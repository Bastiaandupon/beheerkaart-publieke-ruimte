/*
 * Selectie van BRK (grond)percelen waarvan gemeente amsterdam de eigenaar is
 * 
 * m.u.v. opstal nuts en b.p. recht
 * 
 * 
 */
drop table if exists "openbare_ruimte_CROW".eigendom_amsterdam_onbelast;
create table "openbare_ruimte_CROW".eigendom_amsterdam_onbelast as(
--A
select 
    distinct 
    kot.id,
...
drop index if exists "openbare_ruimte_CROW".eigendom_amsterdam_onbelast_gindx;
CREATE INDEX eigendom_amsterdam_onbelast_gindx ON "openbare_ruimte_CROW".eigendom_amsterdam_onbelast USING GIST (poly_geom);

vacuum analyze "openbare_ruimte_CROW".eigendom_amsterdam_onbelast;